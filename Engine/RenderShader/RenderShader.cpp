...
Engine::RenderShader::RenderShader(Engine::Size size)
    : data(new int[size.width * size.height]) {
  camera.size = size;
  CreateTexture();
  CreateRenderShaderProgram();
  UseProgram();
...
Engine::RenderShader::~RenderShader() { delete[] data; }

void Engine::RenderShader::Update() {
  UpdateState(0, 0, camera.size.width, camera.size.height);
  UpdateRenderShader();
}

void Engine::RenderShader::CreateTexture() noexcept {
...
void Engine::RenderShader::CreateRenderShaderProgram() {
  auto vertexRenderShader = CreateVertexRenderShader();
  auto fragmentRenderShader = CreateFragmentRenderShader();

  program = glCreateProgram();

  glAttachShader(program, vertexRenderShader);
  glAttachShader(program, fragmentRenderShader);
...
void Engine::RenderShader::UseProgram() noexcept {
...
GLuint Engine::RenderShader::CreateVertexRenderShader() const {
  auto source =
      "#version 110\n"
  ...
  auto Rendershader = glCreateShader(GL_VERTEX_SHADER);

  glShaderSource(Rendershader, 1, static_cast<const GLchar **>(&source), &length);
  glCompileShader(Rendershader);

  GLint status;
  glGetShaderiv(Rendershader, GL_COMPILE_STATUS, &status);
  if (!status) {
    throw;
  }
...
GLuint Engine::RenderShader::CreateFragmentRenderShader() const {
  auto source =
      "#version 110 \n"
  ...
  auto length = static_cast<int>(strlen(source));
  auto Rendershader = glCreateShader(GL_FRAGMENT_SHADER);
...
void Engine::RenderShader::UpdateRenderShader() const noexcept {
  glBindTexture(GL_TEXTURE_2D, texture);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, camera.size.width,
               camera.size.height, 0, GL_BGRA, GL_UNSIGNED_BYTE, data);
...
void Engine::RenderShader::GetDeviceId() {
  int err;
...
void Engine::RenderShader::CreateContext() {
  int err;
...
void Engine::RenderShader::CreateCommandQueue() {
  int err;
...
void Engine::RenderShader::CreateKernel() {
  int err;

  auto program = CreateProgram();

  err = clBuildProgram(program, 1, &device, "-I ../Engine/RenderShader", NULL, NULL);
  if (err != 0) {
    size_t size;
...
cl_program Engine::RenderShader::CreateProgram() {
  cl_program program;
  char **program_buf;
  size_t *program_size;
  int files_num;
...
void Engine::RenderShader::ReadFilesToBuffer(char **program_buf,
                                       size_t *program_size) {
  program_buf[0] =
      ReadFileToBuffer("../Engine/RenderShader/kernel.cl", &program_size[0]);
}

char *Engine::RenderShader::ReadFileToBuffer(const char *name, size_t *program_size) {
  size_t file_size;
  FILE *fd;
  char *buf;
...
void Engine::RenderShader::LoadMem() {
  int err;
...
void Engine::RenderShader::SetArgument() {
  int err;
...
void Engine::RenderShader::UpdateState(int x, int y, int ox, int oy) const {
  int err;
  ...
  c_camera.a11 = camera.matrix[0][0];
  c_camera.a12 = camera.matrix[0][1];
  c_camera.a13 = camera.matrix[0][2];

  c_camera.a21 = camera.matrix[1][0];
  c_camera.a22 = camera.matrix[1][1];
  c_camera.a23 = camera.matrix[1][2];

  c_camera.a31 = camera.matrix[2][0];
  c_camera.a32 = camera.matrix[2][1];
  c_camera.a33 = camera.matrix[2][2];
...
void Engine::RenderShader::LoadMesh(TriangleMesh mesh) {
  int err;
